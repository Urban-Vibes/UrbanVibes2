...
$directorioRaiz = '/var/www/html'; // Usa la ubicación actual del script como raíz
...
if (isset($_GET['abrir']) && !empty($_GET['abrir'])) {
    ...
    if (file_exists($rutaArchivo) && is_file($rutaArchivo)) {
        ...
        echo "<br><a href='?dir=$relativePath'>Volver</a>";
        exit;
    }
}
...
if (isset($_POST['guardar']) && isset($_POST['archivo']) && isset($_POST['contenido'])) {
    ...
    if (file_exists($rutaArchivo) && is_file($rutaArchivo)) {
        ...
        if (is_writable($rutaArchivo)) {
            ...
            header("Location: ?dir=$relativePath&guardado=1");
            exit;
        } else {
...
<body>
    <h1 style="color: red;">Explorador de Archivos</h1>
    ...
    <div class="container">
            ...
            <div class="breadcrumb">
                ...
                if (!empty($relativePath)) {
                    ...
                    foreach ($partes as $parte) {
                        ...
                        echo ' / <a href="?dir=$rutaAcumulada">' . htmlspecialchars($parte) . '</a>';
                    }
                }
                ?>
            </div>
            ...
            <table class="hola">
                ...
                ?>
                    <tr>
                        <td>
                            <?php if ($esCarpeta): ?>
                                📁 <a href="?dir=<?=$rutaRelativa ?>"><?= htmlspecialchars($elemento) ?></a>
                            <?php else: ?>
                        ...
                        <td>
                            <?php if (!$esCarpeta): ?>
                                <form method="GET" style="display: inline;">
                                    <input type="hidden" name="dir" value="<?= $relativePath ?>">
                                    <input type="hidden" name="abrir" value="<?= $elemento ?>">
        ...
        <?php if ($modoEdicion): ?>
        <div class="editor">
            <h2 style="color: red;">Editando: <?= htmlspecialchars($archivoParaEditar) ?></h2>
            <form method="POST">
